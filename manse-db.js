/* ================================================
   manse-db.js  |  WEROS Life Report
   만세력 + 신살 계산 라이브러리
   기반: lunar-javascript (MIT License)
   검증: 1973-09-19 10:30 → 계축/신유/무오/정사 ✓
   ================================================ */
...
(function(root) {
  ...
  var ZHI_ARR = ['자','축','인','묘','진','사','오','미','신','유','술','해'];
  ...
  var NAYIN_KO = {
    ...
    '覆灯火':'복등화','天河水':'천하수','大驿土':'대역토','钗钏金':'채천금',
    '桑柘木':'상자목','大溪水':'대계수','砂中土':'사중토','天上火':'천상화',
    '石榴木':'석류목','大海水':'대해수','沙中土':'사중토','大林木':'대림목'
  };

  function toKo(h) {
    if (!h) return h;
    return h.split('').map(function(c){ return GAN_KO[c] || ZHI_KO[c] || c; }).join('');
  }
  function ssKo(s) {
    if (!s) return s;
    if (Array.isArray(s)) return s.map(function(x){ return SHISHEN_KO[x]||x; });
    return SHISHEN_KO[s] || s;
  }
  function csKo(s) { return CHANGSHENG_KO[s] || s; }
  function nyKo(s) { return NAYIN_KO[s] || s; }

  // ── 신살 계산 ───────────────────────────────────
  // 역마: 인오술→신, 사유축→해, 신자진→인, 해묘미→사
  var YEOKMA  = {'인':8,'오':8,'술':8,'사':11,'유':11,'축':11,'신':2,'자':2,'진':2,'해':5,'묘':5,'미':5};
  // 도화: 인오술→묘, 사유축→오, 신자진→유, 해묘미→자
  var DOHWA   = {'인':3,'오':3,'술':3,'사':6,'유':6,'축':6,'신':9,'자':9,'진':9,'해':0,'묘':0,'미':0};
  // 화개: 인오술→술, 사유축→축, 신자진→진, 해묘미→미
  var HWAGAE  = {'인':10,'오':10,'술':10,'사':1,'유':1,'축':1,'신':4,'자':4,'진':4,'해':7,'묘':7,'미':7};
  // 천을귀인 (일간 기준)
  var CHUNEUL = {
    '갑':['축','미'],'무':['축','미'],'경':['축','미'],
    '을':['자','신'],'기':['자','신'],
  ...
  // 양인 (일간 기준)
  var YANGIN = {'갑':'묘','병':'오','무':'오','경':'유','임':'자','을':'진','정':'미','기':'미','신':'술','계':'축'};
  // 백호대살 (일주 기준)
  var BAEKHO = ['갑진','을미','병술','정축','무진','임술','계축'];
  // 괴강살
  var GOEGANG = ['경진','경술','임진','무술'];
  // 고란살
  var GORAN = ['갑인','을사','무신','신해','임자'];
  // 천간합
  var GAN_HAP = [['갑','기'],['을','경'],['병','신'],['정','임'],['무','계']];
  // 지지 육합
  var ZHI_YUGHAP = [['자','축'],['인','해'],['묘','술'],['진','유'],['사','신'],['오','미']];
  // 삼합
  var SAMHAP = [['인','오','술'],['사','유','축'],['신','자','진'],['해','묘','미']];
  // 충
  var CHUNG = [['자','오'],['축','미'],['인','신'],['묘','유'],['진','술'],['사','해']];
  // 형
  var HYUNG = [['자','묘'],['인','사','신'],['축','술','미'],['진'],['오'],['유'],['해']];

  function calcSinsal(일간, 일지, 년지, allGan, allZhi) {
    var result = {
      역마: null, 역마_있음: false,
      도화: null, 도화_있음: false,
      화개: null, 화개_있음: false,
      천을귀인: [], 천을귀인_있음: false,
      문창귀인: null, 문창귀인_있음: false,
      양인: null, 양인_있음: false,
      백호대살: false,
      괴강살: false,
      고란살: false,
    ...
    // 역마 (일지 기준)
    var ym = ZHI_ARR[YEOKMA[일지]];
    result.역마 = ym;
    result.역마_있음 = allZhi.indexOf(ym) >= 0;

    // 도화 (일지 기준)
    var dh = ZHI_ARR[DOHWA[일지]];
    result.도화 = dh;
    result.도화_있음 = allZhi.indexOf(dh) >= 0;

    // 화개 (일지 기준)
    var hg = ZHI_ARR[HWAGAE[일지]];
    result.화개 = hg;
    result.화개_있음 = allZhi.indexOf(hg) >= 0;

    // 천을귀인
    var ce = CHUNEUL[일간] || [];
    result.천을귀인 = ce;
    result.천을귀인_있음 = ce.filter(function(z){ return allZhi.indexOf(z) >= 0; }).length > 0;
    result.천을귀인_해당 = ce.filter(function(z){ return allZhi.indexOf(z) >= 0; });

    // 문창귀인
    var mc = MUNCHANG[일간];
    result.문창귀인 = mc;
    result.문창귀인_있음 = allZhi.indexOf(mc) >= 0;

    // 양인
    var yi = YANGIN[일간];
    result.양인 = yi;
    result.양인_있음 = allZhi.indexOf(yi) >= 0;

    // 백호대살
    result.백호대살 = BAEKHO.indexOf(일간+일지) >= 0;

    // 괴강살
    result.괴강살 = GOEGANG.indexOf(일간+일지) >= 0;

    // 고란살
    result.고란살 = GORAN.indexOf(일간+일지) >= 0;

    // 천간합
    GAN_HAP.forEach(function(pair){
      if(allGan.indexOf(pair[0]) >= 0 && allGan.indexOf(pair[1]) >= 0){
        result.천간합.push(pair[0]+pair[1]+'합');
      }
    });

    // 지지 육합
    ZHI_YUGHAP.forEach(function(pair){
      if(allZhi.indexOf(pair[0]) >= 0 && allZhi.indexOf(pair[1]) >= 0){
        result.지지육합.push(pair[0]+pair[1]+'합');
      }
    });

    // 삼합
    SAMHAP.forEach(function(group){
      var found = group.filter(function(z){ return allZhi.indexOf(z) >= 0; });
      if(found.length >= 2){
        result.삼합.push(group.join('')+'('+found.join(',')+')');
      }
    });
    ...
    CHUNG.forEach(function(pair){
      if(allZhi.indexOf(pair[0]) >= 0 && allZhi.indexOf(pair[1]) >= 0){
        result.충.push(pair[0]+pair[1]+'충');
      }
    });

    // 형
    HYUNG.slice(0,3).forEach(function(group){
      var found = group.filter(function(z){ return allZhi.indexOf(z) >= 0; });
      if(found.length >= 2){
        result.형.push(group.join('')+'형('+found.join(',')+')');
      }
    });
  ...
  // ── 메인 함수 ───────────────────────────────────
  function calcSaju(year, month, day, hour, minute, isLunar, gender) {
    var Solar, LunarCls;
    if (typeof window !== 'undefined') {
      Solar = window.Solar; LunarCls = window.Lunar;
    } else {
      var lib = require('/tmp/node_modules/lunar-javascript');
      Solar = lib.Solar; LunarCls = lib.Lunar;
    }

    var solar;
    if (isLunar) {
      solar = LunarCls.fromYmdHms(year, month, day, hour||0, minute||0, 0).getSolar();
    } else {
      solar = Solar.fromYmdHms(year, month, day, hour||0, minute||0, 0);
    }

    var bazi = solar.getLunar().getEightChar();
    var hasTime = (hour !== null && hour !== undefined && hour !== '' && hour !== '모름');

    // 기본 사주
    var 년간 = toKo(bazi.getYearGan());
    var 년지 = toKo(bazi.getYearZhi());
    var 월간 = toKo(bazi.getMonthGan());
    var 월지 = toKo(bazi.getMonthZhi());
    var 일간 = toKo(bazi.getDayGan());
    var 일지 = toKo(bazi.getDayZhi());
    var 시간 = hasTime ? toKo(bazi.getTimeGan()) : '-';
    var 시지 = hasTime ? toKo(bazi.getTimeZhi()) : '-';

    var allGan = [년간, 월간, 일간, 시간];
    var allZhi = [년지, 월지, 일지, 시지];

    // 공망
    var 공망원 = bazi.getDayXunKong();
    var 공망 = toKo(공망원);

    // 대운
    var genderNum = (gender === 'F' || gender === '여성') ? 0 : 1;
    var yun = bazi.getYun(genderNum);
    var daYunList = yun.getDaYun().slice(0,9).map(function(dy){
      return {
        startAge: dy.getStartAge(),
        endAge:   dy.getEndAge(),
        ganzhi:   dy.getGanZhi(),
        ganzhiKo: toKo(dy.getGanZhi()),
    ...
    // 신살
    var sinsal = calcSinsal(일간, 일지, 년지, allGan, allZhi);
    sinsal.공망 = 공망;

    return {
      // 기본 사주
      year:  { gan: 년간, zhi: 년지, full: 년간+년지, han: bazi.getYear() },
      month: { gan: 월간, zhi: 월지, full: 월간+월지, han: bazi.getMonth() },
      day:   { gan: 일간, zhi: 일지, full: 일간+일지, han: bazi.getDay() },
      time:  { gan: 시간, zhi: 시지, full: hasTime ? 시간+시지 : '모름', han: hasTime ? bazi.getTime() : '-' },
      dayGan: 일간,
      ...
      wuxing: {
        ...
        time:  hasTime ? bazi.getTimeWuXing() : '-'
      },

      // 십신
      shishen: {
        yearGan:  ssKo(bazi.getYearShiShenGan()),
        yearZhi:  ssKo(bazi.getYearShiShenZhi()),
        monthGan: ssKo(bazi.getMonthShiShenGan()),
        monthZhi: ssKo(bazi.getMonthShiShenZhi()),
        dayZhi:   ssKo(bazi.getDayShiShenZhi()),
        timeGan:  hasTime ? ssKo(bazi.getTimeShiShenGan()) : '-',
        timeZhi:  hasTime ? ssKo(bazi.getTimeShiShenZhi()) : '-'
      },

      // 지장간
      hideGan: {
        year:  bazi.getYearHideGan().map(toKo),
        month: bazi.getMonthHideGan().map(toKo),
        day:   bazi.getDayHideGan().map(toKo),
        time:  hasTime ? bazi.getTimeHideGan().map(toKo) : []
      },

      // 십이운성
      changSheng: {
        year:  csKo(bazi.getYearDiShi()),
        month: csKo(bazi.getMonthDiShi()),
        day:   csKo(bazi.getDayDiShi()),
        time:  hasTime ? csKo(bazi.getTimeDiShi()) : '-'
      },

      // 납음
      naYin: {
        year:  nyKo(bazi.getYearNaYin()),
        month: nyKo(bazi.getMonthNaYin()),
        day:   nyKo(bazi.getDayNaYin()),
        time:  hasTime ? nyKo(bazi.getTimeNaYin()) : '-'
      },
      ...
      // 태원/명궁
      taiYuan: toKo(bazi.getTaiYuan()),
      mingGong: toKo(bazi.getMingGong()),
      shenGong: toKo(bazi.getShenGong()),
    };
  }
